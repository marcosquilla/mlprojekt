...
class CarDataModule_s1(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage in (None, "fit"):
            self.train_data = CarDataset_s1(self.train_idx, time_window=self.time_window)
        if stage in (None, "test"):
            self.test_data = CarDataset_s1(self.test_idx, time_window=self.time_window)
    ...
    def prepare_data(self, rental_folder:str='SN rentals', open_folder:str='SN App requests', optimise=False, opt_zones=False, n_zones=20):
        # Limits of modelling
        ...
        if not (Path('.') / 'data' / 'processed' / 'demand.csv').is_file():
            ...
            pd.DataFrame(pd.Series('Time').append(pd.Series([i for i in range(n_zones)]))).T.to_csv(Path('.') / 'data' / 'processed' / 'demand.csv', header=False, index=False) # Create csv with header
...
class CarDataModule_s2(pl.LightningDataModule):
    def __init__(self, batch_size:int=16, time_step:timedelta=timedelta(minutes=30), time_window:timedelta=timedelta(minutes=30),
    time_start=datetime(2020, 2, 1, 0, 56, 26), time_end=datetime(2021, 5, 3, 23, 59, 55), 
    test_size=0.2, shuffle=False, num_workers=0, n_zones=20):
        super().__init__()
        ...
        self.timepoints = np.arange(time_start, time_end, time_step).astype(datetime)
        self.prepare_data(n_zones=n_zones)
        self.indices = pd.MultiIndex.from_frame(pd.read_csv((Path('.') / 'data' / 'processed' / 'actions.csv'), usecols=['Time', 'Vehicle_Number_Plate'], parse_dates=['Time'])).reorder_levels([1,0])

        self.train_idx, self.test_idx = train_test_split(self.indices, test_size=test_size, shuffle=shuffle)

    def setup(self, stage=None):
        if stage in (None, "fit"):
            self.train_data = CarDataset_s2(self.train_idx, time_window=self.time_window)
        if stage in (None, "test"):
            self.test_data = CarDataset_s2(self.test_idx, time_window=self.time_window)
